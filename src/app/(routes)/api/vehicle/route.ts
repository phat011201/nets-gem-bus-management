    ...
    const formattedVehicles = vehicles.map(
      (vehicle: {
        ...
        transports: Array<{
          ...
          ticketSeller: { name: string | null } | null;
        ...
        return {
          ...
          operator: transport.operator?.name || null,
          ticketSeller: transport.ticketSeller?.name || null,
...
export async function PUT(request: Request) {
  try {
    ...
    if (!id || !licensePlate || !status) {
      return new Response(
        JSON.stringify({
          message: 'ID, license plate and status are required',
        }),
        { status: 400, headers: { 'Content-Type': 'application/json' } },
      );
    }
